...
- # Research a document
  - The short path: drop a PDF onto a page, read it in a companion pane, and highlight as you go. Each highlight becomes an ordinary bullet you can write under, quote anywhere, and follow back to the exact spot — so a paper turns into linked, searchable notes without leaving Tine. The precise highlight and storage rules live in [[tine-guide/Features/PDF annotation]].
...
- ## Where next
  - The exact highlight, merge, and storage rules are mapped in [[tine-guide/Features/PDF annotation]]. For quoting anything else with `((…))` and the reference panels, see [[tine-guide/Workflows/Find and revisit]]; for snapshots and backups, [[tine-guide/Reference/Files, external edits, and backups]].
