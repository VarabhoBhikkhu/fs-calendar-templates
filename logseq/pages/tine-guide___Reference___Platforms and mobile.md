...
- ## Question 2 — which app is running?
  ...
  - The desktop apps (Linux, macOS, Windows) keep the full window model — tabs, pinned tabs, split panes, saved workspaces, and multiple windows — toured in [[tine-guide/Workflows/Keep context visible]].
...
- ## On Android
  ...
  - **Plugins and themes follow the same lifecycle as desktop** — browse, install (always disabled at first), review its capabilities, enable, settings, disable, uninstall — and a package that did not declare Android shows **Unavailable** on it. The hands-on path is [[tine-guide/Workflows/Extend Tine]].
...
- ## What stays different on Android
  - **Global quick capture is desktop-only** — the `tine --capture` desktop shortcut and its always-on-top capture window have no Android counterpart. Its setup lives on [[tine-guide/Features/Quick capture]].
...
- ## Where next
  - [[tine-guide/Workflows/Keep context visible]] for tabs, panes, sidebars and workspaces (desktop), [[tine-guide/Features/Quick capture]] for the desktop capture window, [[tine-guide/Workflows/Extend Tine]] for the per-platform plugin and theme lifecycle, and [[tine-guide/Features/PDF annotation]] for the full reader tour.
