...
- ## Optional: add a derived field
	- If Estimate needs a computed companion, add a formula from the table header. For the visual formula editor, its read-only results, and the raw-expression escape hatch, use [[tine-guide/Features/Formulas]]. For table, board, and query-view details, return to [[tine-guide/Features/Sheets]].
